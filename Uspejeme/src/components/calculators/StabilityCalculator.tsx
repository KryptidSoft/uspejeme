...
const results = useMemo(() => calculateStability(inputs), [inputs]);
  ...
  useEffect(() => {
    // Kontrola existence results a score
    if (results && typeof results.score === 'number') {
      if (data.stability !== results.score) {
        updateData({ stability: results.score });
      }
    }
    // Linter vyžaduje i 'data.stability' a 'updateData', 
    // protože jsou použity uvnitř efektu.
  }, [results, data.stability, updateData]); 
  ...
  <div style={{ textAlign: 'center' }}>
    <h1>Jak neprůstřelné je vaše podnikání?</h1>
    <h2>
